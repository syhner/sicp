...
we want the stream to compute itself incrementally, so that we only need to process as much of the stream as necessary (as opposed to the whole stream at each stage). we want expressions that can be `delay`ed to create a promise (delayed procedure) which is only computed once `force`d
