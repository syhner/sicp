...
queries are implemented as a box which takes two input streams
- input stream: dictionaries
- input stream: database (collection of facts)
...
means of combination is achieved by connecting queries. importantly, combined queries have the same shape as individual queries: 2 inputs streams (can be fanned out to subqueries) and 1 output stream, and so we have closure. e.g.
